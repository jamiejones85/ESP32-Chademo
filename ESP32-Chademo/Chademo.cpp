...
void CHADEMO::loop()
{
  ...
  if (digitalRead(CHADEMO_IN1)) //IN1 goes HIGH if we have been plugged into the chademo port
  {
    if (insertionTime == 0)
    {
  ...
  if (bChademoMode)
  {
    ...
    switch (chademoState)
    {
      ...
      case WAIT_FOR_BEGIN_CONFIRMATION:
        if (!digitalRead(CHADEMO_IN2)) //inverse logic from how IN1 works. Be careful!
        {
      ...
      case CLOSE_CONTACTORS:
        ...
        digitalWrite(CHADEMO_OUT3, HIGH);
...
void CHADEMO::sendCANBattSpecs()
{
  ...
  #endif
  #ifndef SIMPBMS
  outFrame.data[6] = (uint8_t)settings.packSizeKWH; //charged_rate_reference (change to SoC from BMS)
  #endif
